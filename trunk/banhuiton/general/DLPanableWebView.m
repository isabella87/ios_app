...
@interface DLPanableWebView()<UIWebViewDelegate, MBProgressHUDDelegate>
...
- (void)goBack{
    if ([self.currentURL containsString: @"app/mobile/settingsN.html"] || [self.currentURL containsString: @"pinset/jump2CP"]) {
        [super goBack] ;
        [super goBack] ;
    } else if ([self.currentURL containsString: @"app/mobile/withdrawN"] || [self.currentURL containsString: @"withhold/jump2CP"]){
        [super goBack] ;
        [super goBack] ;
    } else {
        [super goBack] ;
...
- (id)init{
    if (self = [super init]) {
        ...
        HUD.delegate = self ;
        self.scrollView.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{[self reload] ;}] ;
        [self commonInit];
    }
    return self;
}
